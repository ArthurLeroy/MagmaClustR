# ==========================================================================
# Benchmark_Phase1_NC_MOMT.R
# Phase 1 NC : Test paramétrique — MOMT sans clustering (Magma multi-output)
#
# Usage :
#   Rscript Benchmark_Phase1_NC_MOMT.R --param=probleme --config=default --seed=1
#
# Paramètres possibles :
#   --param   : probleme, hp_clusters, hp_tasks, inputs, n_train
#   --config  : default, variation
#   --seed    : 1..10
#
# Ce script génère les données (simulate_multi_output_data_convol),
# entraîne Magma multi-output (train_magma), et produit les prédictions.
# Les scripts MO et MT NC réutilisent les données générées ici.
# ==========================================================================
...
if (CONFIG == "variation") {
  switch(PARAM,
    ...
    "inputs"      = { SHARED_GRID_OUTPUTS <- TRUE },
    "n_train"     = { N_TRAIN <- 100 }
  )
}
...
cat(paste0("=== Phase 1 NC MOMT | param=", PARAM, " config=", CONFIG, " seed=", SEED, " ===\n"))
...
run_info <- list(
  script = "Benchmark_Phase1_NC_MOMT.R",
...
generate_random_config <- function(n_out, shared_hp_tasks = TRUE) {
  hp_mp_list <- list()
  jitter_leng <- 0.2
  jitter_var  <- 0.5
  ...
  for (o in 1:n_out) {
    ...
    # Un seul jeu de HP par output (pas de clusters)
  ...
  hp_task_config <- bind_rows(base_task_params)
...
tryCatch({
  ...
  configs   <- generate_random_config(N_OUT, shared_hp_tasks = SHARED_HP_TASKS)
  ...
  for (o_id in 1:N_OUT) {
    ...
    for (seed_retry in 1:10) {
      tryCatch({
        ...
        cat(paste0("  [train_gp ERR] o=", o_id,
                   " seed=", seed_retry, ": ", e$message, "\n"))
      })
    }

    if (is.null(best_hp_gp)) {
      cat(paste0("  [ATTENTION] Aucun train_gp n'a réussi pour o=", o_id,
                 ". Valeurs par défaut.\n"))
    ...
    hp_0_extracted_list[[length(hp_0_extracted_list) + 1]] <- tibble(
      Output_ID = as.factor(o_id),
      l_t = l_val, S_t = v_val, noise = best_hp_gp$noise, prior_mean = mean_emp
    )
  }

  ini_hp_0_raw <- bind_rows(hp_0_extracted_list)
  ini_hp_0_raw$noise <- -3
  ini_hp_0 <- ini_hp_0_raw %>%
    dplyr::mutate(l_u_t = max_lengthscale_observed) %>%
    dplyr::select(-prior_mean)

  # --- 7. INITIALISATION TÂCHES ---
  hp_0_for_init <- ini_hp_0 %>%
  ...
  ini_hp_t <- hp(
    ...
    hp_config = tibble(
      output_id = hp_0_for_init$Output_Num,
      lt_min = hp_0_for_init$l_t, lt_max = hp_0_for_init$l_t,
      St_min = rep(0, N_OUT), St_max = rep(0, N_OUT),
      lu_min = hp_0_for_init$l_u_t, lu_max = hp_0_for_init$l_u_t,
      noise_min = -3, noise_max = -3
    )
  )

  # --- 8. ENTRAÎNEMENT MAGMA (sans clustering) ---
  prior_means_vec <- ini_hp_0_raw %>%
  ...
  # Sauvegarde des datasets (pour MO et MT NC)
  ...
  cat(paste0("\n!!! ERREUR [NC MOMT param=", PARAM, " config=", CONFIG, " seed=", SEED, "] :\n"))
...
cat(paste0("\n=== NC MOMT TERMINÉ [param=", PARAM, " config=", CONFIG, " seed=", SEED, "] ===\n"))
