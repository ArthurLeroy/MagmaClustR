# ==========================================================================
# Benchmark_Phase1_NC_MT.R
# Phase 1 NC : Test paramétrique — MT sans clustering (Magma single-output)
#
# Usage :
#   Rscript Benchmark_Phase1_NC_MT.R --param=probleme --config=default --seed=1
#
# Charge les données/modèle MOMT NC générés par Benchmark_Phase1_NC_MOMT.R.
# Entraîne Magma (train_magma) sur l'Output 1 uniquement (kernel SE).
# ==========================================================================
...
cat(paste0("=== Phase 1 NC MT | param=", PARAM, " config=", CONFIG, " seed=", SEED, " ===\n"))
...
run_info <- list(
  script = "Benchmark_Phase1_NC_MT.R",
...
tryCatch({
  ...
  # Extraction HP initiaux depuis le modèle MOMT NC
  # hp_0 du modèle Magma (convolution_kernel) → convertir en SE pour single-output
  ini_hp_0 <- trained_model_momt$hp_0 %>%
    dplyr::filter(Output_ID == "1") %>%
  ...
  # Prior mean pour Output 1 : moyenne empirique des données training
  prior_mean_o1 <- mean(train_data_o1$Output, na.rm = TRUE)

  # --- 5. ENTRAÎNEMENT MAGMA SINGLE-OUTPUT ---
  t_train_start <- Sys.time()
  trained_model_mt <- train_magma(
    data            = train_data_o1,
    prior_mean      = prior_mean_o1,
    ini_hp_0        = ini_hp_0 %>% dplyr::select(-any_of("noise")),
    ini_hp_t        = ini_hp_t,
    kern_0          = "SE",
    kern_t          = "SE",
    shared_hp_tasks = SHARED_HP_TASKS,
  ...
  cat(paste0("\n!!! ERREUR [NC MT param=", PARAM, " config=", CONFIG, " seed=", SEED, "] : ", e$message, "\n"))
...
cat(paste0("\n=== NC MT TERMINÉ [param=", PARAM, " config=", CONFIG, " seed=", SEED, "] ===\n"))
