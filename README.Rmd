...
set.seed(17)
data_magma <- simu_data(n_tasks = 11, n_points = 10, shared_input = FALSE)
## Split individuals into training and prediction sets, and define test points
magma_train <- data_magma %>% subset(Task_ID %in% 1:10)
magma_pred <- data_magma %>% subset(Task_ID == 11) %>% head(7)
magma_test <- data_magma %>% subset(Task_ID == 11) %>% tail(3)
...
Note that the `shared_hp` and `grid_inputs` arguments are optional. They respectively indicate that a specific set of hyper-parameters is trained for each curve, and control the grid of values on which the prediction is performed.
