...
test_that("gradient of logL_GP_mod_common_hp() works", {
  ...
  deriv_v = gr_GP_mod_common_hp(hp, db, mean,
                                'SE', new_cov, 0.1)[['se_variance']]
  deriv_l = gr_GP_mod_common_hp(hp, db, mean,
                                'SE', new_cov, 0)[['se_lengthscale']]
...
test_that("gradient of gr_sum_logL_GP_clust() works for SE kernel", {
  ...
  deriv_v = gr_sum_logL_GP_clust(hp, db, mixture, mean,
                                 'SE', new_cov, 0)[['se_variance']]
  deriv_l = gr_sum_logL_GP_clust(hp, db, mixture, mean,
                                 'SE', new_cov, 0)[['se_lengthscale']]

  emp_deriv_v = (sum_logL_GP_clust(hp_v, db, mixture, mean,
                                   'SE', new_cov, NULL, 0) -
    sum_logL_GP_clust(hp, db, mixture, mean, 'SE', new_cov, NULL, 0)) / 10^(-8)
  emp_deriv_l = (sum_logL_GP_clust(hp_l, db, mixture, mean,
                                   'SE', new_cov, NULL, 0) -
    sum_logL_GP_clust(hp, db, mixture, mean, 'SE', new_cov, NULL, 0)) / 10^(-8)
