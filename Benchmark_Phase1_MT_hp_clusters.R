# ==========================================================================
# Benchmark_Phase1_MT_hp_clusters.R
# Phase 1 : Test paramétrique — MT dédié au paramètre "hp_clusters"
#
# Usage :
#   Rscript Benchmark_Phase1_MT_hp_clusters.R --config=default --seed=1
#
# Configs :
#   default   → shared_hp_clusts = FALSE  (distinct)
#   variation → shared_hp_clusts = TRUE   (shared)
#
# Charge les données/modèle MOMT générés par Benchmark_Phase1_MOMT_hp_clusters.R.
# Quand shared_hp_clusts = TRUE, les ini_hp_k extraits du modèle MOMT
# (déjà partagés après entraînement) sont passés tels quels.
# ==========================================================================
...
cat(paste0("=== Phase 1 MT hp_clusters | config=", CONFIG, " seed=", SEED, " ===\n"))
...
run_info <- list(
  script = "Benchmark_Phase1_MT_hp_clusters.R",
...
tryCatch({
  ...
  # Extraction HP initiaux depuis le modèle MOMT (déjà partagés si shared)
  ini_hp_k <- trained_model_momt$hp_k %>%
    dplyr::filter(Output_ID == "1") %>%
    dplyr::select(-any_of(c("l_u_t", "prop_mixture"))) %>%
  ...
  cat(paste0("\n!!! ERREUR [MT hp_clusters config=", CONFIG, " seed=", SEED, "] : ", e$message, "\n"))
...
cat(paste0("\n=== MT hp_clusters TERMINÉ [config=", CONFIG, " seed=", SEED, "] ===\n"))
