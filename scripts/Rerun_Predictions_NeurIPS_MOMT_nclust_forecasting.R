# ==========================================================================
# Rerun_Predictions_NeurIPS_MOMT_nclust_forecasting.R
# NeurIPS : relance uniquement les prédictions MOMT (forecasting)
# à partir des datasets et modèles déjà sauvegardés sur le serveur.
#
# Usage :
#   Rscript Rerun_Predictions_NeurIPS_MOMT_nclust_forecasting.R \
#     --n_out=2 --n_train=30 --n_pred=1 --n_clust=1 \
#     --problem=forecasting --seed=1
# ==========================================================================
...
for (d in c(dir_predictions_momt, dir_run_info)) {
  if (!dir.exists(d)) dir.create(d, recursive = TRUE)
}

run_info <- list(
  script = "Rerun_Predictions_NeurIPS_MOMT_nclust_forecasting.R",
  n_out = N_OUT,
  n_train = N_TRAIN,
  n_pred = N_PRED,
  n_clust = N_CLUST,
...
run_info_file <- file.path(
  dir_run_info,
  paste0("rerun_pred_momt_seed", SEED, "_", format(Sys.time(), "%Y%m%d_%H%M%S"), ".json")
)
...
tryCatch({
  ...
  trained_model       <- model_momt$trained_model
  ...
  if (is.null(trained_model)) stop("Objet trained_model absent du fichier modèle MOMT.")
  if (is.null(pred_tasks_data_all) || is.null(test_tasks_data_all)) {
    stop("Les splits pred/test sont absents du fichier datasets MOMT.")
  }
  ...
  for (test_task_id in test_task_ids) {
    ...
    pred_input <- pred_tasks_data_all[[test_task_id]] %>%
  ...
  predictions_to_save <- list(
    ...
    t_training         = model_momt$t_training,
    seed               = SEED,
    n_clust            = N_CLUST,
