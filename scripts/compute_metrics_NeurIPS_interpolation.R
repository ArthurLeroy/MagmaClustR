# ==========================================================================
# compute_metrics_NeurIPS_interpolation.R
# NeurIPS : Calcul des metriques pour l'interpolation uniquement (MOMT + MT)
#
# Usage :
#   Rscript compute_metrics_NeurIPS_interpolation.R
#
# Base sur compute_orginal.R (qui extrait correctement MOMT et MT).
# Ajout du parametre n_clust et gestion du format pred_magma() (n_clust=1).
#
# IMPORTANT : Les metriques sont calculees sur OUTPUT 1 uniquement,
# en espace original (de-scale pour MOMT).
#
# Sorties :
#   - metrics_raw_interpolation_NeurIPS.csv
#   - metrics_summary_interpolation_NeurIPS.csv
#   - metrics_display_interpolation_NeurIPS.csv
# ==========================================================================
...
extract_metrics_momt <- function(pred_entry, scale_factors) {
  tryCatch({
    ...
    if (nrow(truth_o1) == 0) return(tibble(rmse = NA, nll = NA, nll_multi = NA, coverage_95 = NA))
...
base_dir   <- file.path("/scratch", username, "NeurIPS_experiments_interpolation")
...
if (nrow(metrics_raw) == 0) {
  stop("Aucune prediction interpolation exploitable trouvee dans les repertoires configures.")
}

write_csv(metrics_raw, file.path(output_dir, "metrics_raw_interpolation_NeurIPS.csv"))
saveRDS(metrics_raw, file.path(output_dir, "metrics_raw_interpolation_NeurIPS.rds"))
...
write_excel_csv2(metrics_summary, file.path(output_dir, "metrics_summary_interpolation_NeurIPS.csv"))
write_excel_csv2(metrics_display, file.path(output_dir, "metrics_display_interpolation_NeurIPS.csv"))
saveRDS(metrics_summary, file.path(output_dir, "metrics_summary_interpolation_NeurIPS.rds"))

cat("\n--- Resume interpolation par parametre d'interet ---\n\n")
