# ==========================================================================
# Benchmark_NeurIPS_MO_nclust_interpolation.R
# NeurIPS : Benchmark MO (INTERPOLATION ONLY) avec paramètre n_clust variable
...
dir_models_mo      <- file.path(base_root, "Models_MO", config_label)
dir_predictions_mo <- file.path(base_root, "Predictions_MO", config_label)
...
run_info <- list(
  script = "Benchmark_NeurIPS_MO_nclust_interpolation.R",
...
tryCatch({
  ...
  for (tid in train_task_ids) {
    ...
    # Interpolation : enlever Output_ID="1" dans le gap [-0.5, 0.5]
    task_data <- task_data %>%
  ...
  # 3d. Grid inputs pour la prédiction (Output_ID="1" dans le gap [-0.5, 0.5])
