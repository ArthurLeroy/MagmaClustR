# ==========================================================================
# Benchmark_Phase1_MO.R
# Phase 1 : Test paramétrique — MO (Multi-Output GP sans clustering)
#
# Usage :
#   Rscript Benchmark_Phase1_MO.R --param=probleme --config=default --seed=1
#
# Charge les données générées par le script MOMT.
...
cat(paste0("=== Phase 1 MO | param=", PARAM, " config=", CONFIG, " seed=", SEED, " ===\n"))
...
tryCatch({
  ...
  # HP template depuis le modèle MOMT
  ...
  cat(paste0("\n!!! ERREUR [MO param=", PARAM, " config=", CONFIG, " seed=", SEED, "] : ", e$message, "\n"))
})

# --- Fin ---
duration_total <- as.numeric(difftime(Sys.time(), t_global_start, units = "mins"))
cat(paste0("\n=== MO TERMINÉ [param=", PARAM, " config=", CONFIG, " seed=", SEED, "] ===\n"))
