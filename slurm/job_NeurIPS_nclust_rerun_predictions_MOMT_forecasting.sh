#!/bin/bash -l
#===============================================================================
# job_NeurIPS_nclust_rerun_predictions_MOMT_forecasting.sh
# NeurIPS : relance uniquement les prédictions MOMT forecasting.
#
# 10 configs x 50 seeds = 500 jobs
#
# Soumission : sbatch job_NeurIPS_nclust_rerun_predictions_MOMT_forecasting.sh
#===============================================================================

#SBATCH --job-name=rerun_momt_fcst
#SBATCH --qos=huge
#SBATCH -c 25
#SBATCH --time=7-00:00:00
#SBATCH --output=/scratch/%u/logs/rerun_momt_fcst_%j.out
#SBATCH --error=/scratch/%u/logs/rerun_momt_fcst_%j.err
...
SCRIPT_NAME="Rerun_Predictions_NeurIPS_MOMT_nclust_forecasting.R"
RESULTS_DIR="${PROJECT_ROOT}/Predictions_MOMT"
LOGDIR="/scratch/${USER}/logs/rerun_momt_fcst"

echo "=============================================="
echo " RERUN MOMT FORECASTING"
echo " Date    : $(date)"
echo " Noeud   : $(hostname)"
...
worker() {
  ...
  while true; do
    ...
    LABEL="momt_fcst_out${N_OUT}_train${N_TRAIN}_pred${N_PRED}_clust${N_CLUST}_seed${SEED}"
