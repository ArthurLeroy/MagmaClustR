...
# --- Logs ---
LOGDIR="/scratch/${USER}/logs/neurips_nclust_mo_fc"
mkdir -p "${LOGDIR}"
...
JOBFILE="/scratch/${USER}/logs/neurips_nclust_mo_fc/jobqueue_${SLURM_JOB_ID}.txt"
COUNTERFILE="/scratch/${USER}/logs/neurips_nclust_mo_fc/counter_${SLURM_JOB_ID}"
...
worker() {
  ...
  while true; do
    ...
    ${RSCRIPT} --vanilla "${SCRIPT_DIR}/Benchmark_NeurIPS_MO_nclust_forecasting.R" \
...
echo ""
echo "=============================================="
echo " NeurIPS Phase 2 MO FORECASTING TERMINÉE"
echo " Date       : $(date)"
...
for CONFIG_STR in "${MO_CONFIGS[@]}"; do
  read -r N_OUT N_TRAIN N_PRED N_CLUST N_SEEDS <<< "${CONFIG_STR}"
  for SEED in $(seq 1 ${N_SEEDS}); do
    PRED_FILE="${RESULTS_DIR}/out${N_OUT}_train${N_TRAIN}_pred${N_PRED}_clust${N_CLUST}/forecasting/Predictions_MO/predictions_seed_${SEED}.rds"
    if [ ! -f "${PRED_FILE}" ]; then
...
echo "Vérification OK : toutes les prédictions MO forecasting sont présentes."
exit 0
