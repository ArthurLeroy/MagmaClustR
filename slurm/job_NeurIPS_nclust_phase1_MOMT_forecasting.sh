...
JOBFILE="/scratch/${USER}/logs/neurips_nclust_momt_fc/jobqueue_${SLURM_JOB_ID}.txt"
COUNTERFILE="/scratch/${USER}/logs/neurips_nclust_momt_fc/counter_${SLURM_JOB_ID}"
...
worker() {
  ...
  while true; do
    ...
    ${RSCRIPT} --vanilla "${SCRIPT_DIR}/Benchmark_NeurIPS_MOMT_nclust_forecasting.R" \
...
echo ""
echo "=============================================="
echo " Phase 1 MOMT FORECASTING TERMINÉE"
echo " Date    : $(date)"
...
echo "Vérification OK : toutes les données MOMT forecasting sont présentes."
exit 0
