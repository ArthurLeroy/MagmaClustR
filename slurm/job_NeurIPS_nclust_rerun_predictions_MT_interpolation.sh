#!/bin/bash -l
#===============================================================================
# job_NeurIPS_nclust_rerun_predictions_MT_interpolation.sh
# NeurIPS : relance uniquement les prédictions MT interpolation.
#
# 10 configs x 50 seeds = 500 jobs
#
# Soumission : sbatch job_NeurIPS_nclust_rerun_predictions_MT_interpolation.sh
#===============================================================================

#SBATCH --job-name=rerun_mt_itp
#SBATCH --qos=huge
#SBATCH -c 25
#SBATCH --time=7-00:00:00
#SBATCH --output=/scratch/%u/logs/rerun_mt_itp_%j.out
#SBATCH --error=/scratch/%u/logs/rerun_mt_itp_%j.err
...
SCRIPT_NAME="Rerun_Predictions_NeurIPS_MT_nclust_interpolation.R"
RESULTS_DIR="${PROJECT_ROOT}/Predictions_MT"
LOGDIR="/scratch/${USER}/logs/rerun_mt_itp"

echo "=============================================="
echo " RERUN MT INTERPOLATION"
echo " Date    : $(date)"
echo " Noeud   : $(hostname)"
...
worker() {
  ...
  while true; do
    ...
    LABEL="mt_itp_out${N_OUT}_train${N_TRAIN}_pred${N_PRED}_clust${N_CLUST}_seed${SEED}"
