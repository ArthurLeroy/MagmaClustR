#!/bin/bash -l
#===============================================================================
# job_NeurIPS_nclust_phase2_MO_interpolation.sh
# NeurIPS : Phase 2 — MO INTERPOLATION ONLY
#
# 7 configs × 1 problème (interpolation) × 5 seeds = 35 jobs
#
# Soumission : sbatch job_NeurIPS_nclust_phase2_MO_interpolation.sh
#===============================================================================

#SBATCH --job-name=neurips_nclust_mo_itp
#SBATCH --qos=huge
#SBATCH -c 16
#SBATCH --time=7-00:00:00
#SBATCH --output=/scratch/%u/logs/neurips_nclust_mo_itp_%j.out
#SBATCH --error=/scratch/%u/logs/neurips_nclust_mo_itp_%j.err
...
echo "=============================================="
echo " NeurIPS Phase 2 : MO INTERPOLATION"
echo " Date    : $(date)"
echo " Noeud   : $(hostname)"
...
# --- Logs ---
LOGDIR="/scratch/${USER}/logs/neurips_nclust_mo_itp"
mkdir -p "${LOGDIR}"
...
JOBFILE="/scratch/${USER}/logs/neurips_nclust_mo_itp/jobqueue_${SLURM_JOB_ID}.txt"
COUNTERFILE="/scratch/${USER}/logs/neurips_nclust_mo_itp/counter_${SLURM_JOB_ID}"

> "${JOBFILE}"
for CONFIG_STR in "${MO_CONFIGS[@]}"; do
  read -r N_OUT N_TRAIN N_PRED N_CLUST N_CONFIG_SEEDS <<< "${CONFIG_STR}"
  for SEED in $(seq 1 ${N_CONFIG_SEEDS}); do
...
echo ""
echo "=============================================="
echo " NeurIPS Phase 2 MO INTERPOLATION TERMINÉE"
echo " Date       : $(date)"
...
echo "Vérification OK : toutes les prédictions MO interpolation sont présentes."
exit 0