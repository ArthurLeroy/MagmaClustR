#!/bin/bash -l
#===============================================================================
# job_NeurIPS_nclust_phase2_MO_interpolation_part1.sh
# NeurIPS : Phase 2 — MO INTERPOLATION ONLY (part 1)
#
# 1 config spéciale × seeds {1..5} + 6 configs × seeds {1..25} = 155 jobs
#
# Soumission : sbatch job_NeurIPS_nclust_phase2_MO_interpolation_part1.sh
#===============================================================================

#SBATCH --job-name=neurips_mo_itp_p1
#SBATCH --qos=huge
#SBATCH -c 32
#SBATCH --time=7-00:00:00
#SBATCH --output=/scratch/%u/logs/neurips_mo_itp_p1_%j.out
#SBATCH --error=/scratch/%u/logs/neurips_mo_itp_p1_%j.err
#SBATCH --mail-type=BEGIN,END,FAIL
#SBATCH --mail-user=alexia.grenouillat@math.univ-toulouse.fr

N_WORKERS=32
DEFAULT_SEEDS_PART=$(seq 1 25)
SPECIAL_SEEDS_PART=$(seq 1 5)

echo "=============================================="
echo " NeurIPS Phase 2 : MO INTERPOLATION PART 1"
echo " Date    : $(date)"
echo " Noeud   : $(hostname)"
echo " Job ID  : ${SLURM_JOB_ID}"
echo " CPUs    : ${SLURM_CPUS_ON_NODE}"
echo " Workers : ${N_WORKERS}"
echo " Seeds défaut   : 1..25"
echo " Seeds spéciale : 1..5 pour 4 30 1 1"
echo "=============================================="
...
LOGDIR="/scratch/${USER}/logs/neurips_nclust_mo_itp_part1"
mkdir -p "${LOGDIR}"
...
JOBFILE="/scratch/${USER}/logs/neurips_nclust_mo_itp_part1/jobqueue_${SLURM_JOB_ID}.txt"
COUNTERFILE="/scratch/${USER}/logs/neurips_nclust_mo_itp_part1/counter_${SLURM_JOB_ID}"

> "${JOBFILE}"
for CONFIG_STR in "${MO_CONFIGS[@]}"; do
  read -r N_OUT N_TRAIN N_PRED N_CLUST <<< "${CONFIG_STR}"
  if [ "${N_OUT} ${N_TRAIN} ${N_PRED} ${N_CLUST}" = "4 30 1 1" ]; then
    SEED_RANGE=${SPECIAL_SEEDS_PART}
  else
    SEED_RANGE=${DEFAULT_SEEDS_PART}
  fi
...
echo ""
echo "=============================================="
echo " NeurIPS Phase 2 MO INTERPOLATION PART 1 TERMINÉE"
echo " Date       : $(date)"
...
echo "Vérification OK : toutes les prédictions MO interpolation part 1 sont présentes."
exit 0