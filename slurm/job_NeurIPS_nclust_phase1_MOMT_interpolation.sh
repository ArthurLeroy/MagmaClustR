#!/bin/bash -l
#===============================================================================
# job_NeurIPS_nclust_phase1_MOMT_interpolation.sh
# NeurIPS : Phase 1 — MOMT INTERPOLATION ONLY
#
# 10 configs × 1 problème (interpolation) × 5 seeds = 50 jobs
#
# Soumission : sbatch job_NeurIPS_nclust_phase1_MOMT_interpolation.sh
#===============================================================================

#SBATCH --job-name=neurips_nclust_momt_itp
#SBATCH --qos=huge
#SBATCH -c 16
#SBATCH --time=7-00:00:00
#SBATCH --output=/scratch/%u/logs/neurips_nclust_momt_itp_%j.out
#SBATCH --error=/scratch/%u/logs/neurips_nclust_momt_itp_%j.err
...
# --- Logs ---
LOGDIR="/scratch/${USER}/logs/neurips_nclust_momt_itp"
mkdir -p "${LOGDIR}"
...
UTILS_DIR="/scratch/${USER}/NeurIPS_experiments_interpolation/utils"
...
JOBFILE="/scratch/${USER}/logs/neurips_nclust_momt_itp/jobqueue_${SLURM_JOB_ID}.txt"
COUNTERFILE="/scratch/${USER}/logs/neurips_nclust_momt_itp/counter_${SLURM_JOB_ID}"
...
worker() {
  ...
  while true; do
    ...
    LABEL="out${N_OUT}_train${N_TRAIN}_pred${N_PRED}_clust${N_CLUST}_${PROBLEM}_seed${SEED}"
    LOGFILE="${LOGDIR}/momt_${LABEL}.log"

    echo "[$(date +%H:%M:%S)] Worker ${WORKER_ID} → MOMT ${LABEL}"

    ${RSCRIPT} --vanilla "${SCRIPT_DIR}/Benchmark_NeurIPS_MOMT_nclust_interpolation.R" \
...
echo ""
echo "=============================================="
echo " Phase 1 MOMT INTERPOLATION TERMINÉE"
echo " Date    : $(date)"
...
echo "Vérification OK : toutes les données MOMT interpolation sont présentes."
exit 0