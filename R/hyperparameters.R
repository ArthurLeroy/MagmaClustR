...
#'   (either the fixed defaults, or data-driven bounds if 'data' is provided).
#' @param data An optional tibble or data.frame (columns 'Input', 'Output',
#'   and, for the multi-output case, 'Output_ID'), used to derive data-driven
#'   generation bounds for the parameters whose scale directly depends on the
#'   data (lengthscale, signal variance, noise), instead of the fixed
#'   defaults (which implicitly assume inputs on an \[-1, 1\]-ish domain and
#'   outputs of order 1). Ignored if 'hp_config' is also provided (for the
#'   multi-output case).
#'
...
hp <- function(kern = "SE",
               ...
               hp_config = NULL,
               data = NULL) {
  ## Initiate interval boundaries. When 'data' is provided, the lengthscale/
  ## variance/noise bounds are derived from its actual scale instead of these
  ## fixed defaults.
  min_val_l <- -3
  max_val_l <- 0
  min_val_S <- log(0.5)
  ...
  if (!is.null(data)) {
    b <- .data_scale_bounds(data)
    min_val_l <- b$min_val_l
    max_val_l <- b$max_val_l
    min_val_S <- b$min_val_S
    max_val_S <- b$max_val_S
    min_noise <- b$min_noise
    max_noise <- b$max_noise
  }
  ...
  if (is.function(kern)) {
    ...
    ## or derived from 'data' when provided.
    if (is.null(hp_config) && !is.null(data)) {
...
#' Draw a condensed data-driven initial hyper-parameter set
#'
#' Thin wrapper around [hp()] that always produces a *condensed*
#' hyper-parameter tibble (no 'ID'/'Task_ID' column): a single row for
#' single-output kernels, or one row per 'Output_ID' for the multi-output
#' convolution kernel. Bounds are derived from 'data' (see
#' [.data_scale_bounds()]) rather than the fixed defaults.
#'
#' @param kern A kernel, character string or [convolution_kernel()].
#' @param data A tibble, used to derive data-driven generation bounds.
#'   Required columns: 'Input', 'Output' (and 'Output_ID' for the
#'   multi-output case).
#' @param noise A logical value, whether to draw a 'noise' hyper-parameter.
#' @return A condensed hyper-parameter tibble.
#' @keywords internal
draw_ini_hp <- function(kern, data, noise = TRUE) {
  if (is.function(kern)) {
    out_ids <- unique(as.character(data$Output_ID))
    hp(
      kern,
      list_task_ID = "shared",
      list_output_ID = out_ids,
      shared_hp_tasks = TRUE,
      noise = noise,
      data = data
    ) %>%
