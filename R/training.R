...
train_magma <- function(data,
  ...
  for (i in 1:n_iter_max){
    ...
    if (any(is.na(new_hp_0)) | any(is.na(new_hp_t))) {
...
train_magmaclust <- function(data,
  ...
  if (kern_k %>% is.function()) {
    ...
    if (ini_hp_k %>% is.null()) {
      ...
      ## Rename Task_ID to Cluster_ID for cluster mean processes
      if ("Task_ID" %in% names(hp_k)) {
        hp_k <- hp_k %>% dplyr::rename(Cluster_ID = Task_ID)
      }
      cat(
