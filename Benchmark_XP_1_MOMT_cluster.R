...
# 1. Dossier pour les modèles (dépend uniquement de n_train)
dir_models_momt <- file.path(base_dir, "Models_MOMT", paste0("n_out_", n_out), paste0("train_", n_train))
if (!dir.exists(dir_models_momt)) dir.create(dir_models_momt, recursive = TRUE)

# 2. Dossiers pour les datasets et prédictions (dépendent de n_train ET n_pred)
for (n_pred in n_pred_subsets) {
  task_folder_name <- paste0("train_", n_train, "_pred_", n_pred)
  for (subdir in c("Datasets", "Predictions_MOMT")) {
...
for (iter in 1:n_iterations) {
  ...
  tryCatch({
    ...
    # --- SAUVEGARDE UNIQUE DU MODÈLE POUR CETTE ITÉRATION ---
    dir_models_momt_iter <- file.path(base_dir, "Models_MOMT", paste0("n_out_", n_out), paste0("train_", n_train))
    ...
    saveRDS(model_to_save, file.path(dir_models_momt_iter, paste0("model_iter_", iter, ".rds")))
    # ---------------------------------------------------------
