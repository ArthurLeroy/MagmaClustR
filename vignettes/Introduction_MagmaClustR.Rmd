...
**MAGMACLUST** differs by using more than one common mean processes, and a variational EM algorithm is derived for dealing with the optimization of the hyper-parameters along with the hyper-posteriors’ estimation of latent variables and processes.
...
```{r}
data_train_1D <- simu_db(covariate = FALSE)

training_1D <- train_magma(data = data_train_1D)
```

```{r}
data_pred_1D <- simu_db(M=1, covariate = FALSE)

pred <- pred_magma(data = data_pred_1D, trained_model = training_1D)
```
...
```{r}
data_train_2D <- simu_db(covariate = TRUE)

training_2D <- train_magma(data = data_train_2D)
```

```{r}
data_pred_2D <- simu_db(M=1, covariate = TRUE)

pred <- pred_magma(data = data_pred_2D, trained_model = training_2D)
```
...
```{r}
prediction_gif <- pred_gif(data_pred_1D, trained_model = training_1D)
...
```{r, warning=FALSE}
training_clust_1D <- train_magma_VEM(data = data_train_1D)
```

```{r}
pred_clust_1D <- pred_magma_clust(data_obs = data_pred_1D, trained_magmaclust = training_clust_1D)
```
...
```{r, warning=FALSE}
training_clust_2D <- train_magma_VEM(data = data_train_2D)
```

```{r}
pred_clust_2D <- pred_magma_clust(data_obs = data_pred_2D, trained_magmaclust = training_clust_2D)
```
