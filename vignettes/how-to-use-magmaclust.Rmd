...
library(MagmaClustR)
library(dplyr)
weight = read.csv("data/db_gusto_weight.csv")
weight$ID <- factor(as.numeric(factor(weight$ID)))
...
Throughout this `weight` example, we use MagmaClust to model children’s evolution curves and forecast their future weight. 
...
To get an idea of how our `weight` dataset looks, we display the weight of 5 children according to their age. Do the children’s weight evolve the same, *i.e.* with the same pattern ?  Can we identify group structures in the data, and thus define weight evolution clusters ?


```{r fig.asp = 0.6, fig.width = 7, out.width = "80%"}
ggplot2::ggplot(data=weight %>% filter(ID %in% 1:5),
...
weight_pred <- weight %>% filter(ID == 72) %>% filter(Input<20)
```
...
```{r, eval=FALSE}
selection <- select_nb_cluster(data = weight,
                  grid_nb_cluster = 1:5,
                  kern_k = "SE",
                  kern_i = "SE")
