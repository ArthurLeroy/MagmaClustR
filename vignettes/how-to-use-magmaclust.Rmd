...
set.seed(10)
list_ID <- weight %>% pull(ID) %>% unique() %>% sample()
...
weight_pred <- weight %>% filter(ID == list_ID[323]) %>% filter(Input<20)
weight_test <- weight %>% filter(ID == list_ID[323]) %>% filter(Input>20)
```
...
With the [`plot_magmaclust()`](https://arthurleroy.github.io/MagmaClustR/reference/plot_magmaclust.html) function, we can display the evolution of James' weight. 
...
*   James belongs to one of the clusters with probability 1 (or really close). We can then display the associated 95% credibility interval;
...
The MagmaClustR package also provides an implementation for the *Magma* algorithm, allowing us to compare *MagmaClust* and *Magma* predictions. For more details on Magma, see the dedicated [vignette](how-to-use-magma.html). 
