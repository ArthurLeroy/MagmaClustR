...
For computation time reasons in this illustrative example, we randomly select 20 swimmers for training. Even if the performances of *Magma* increase with the number of training individuals, 20 are more than enough to get a clear idea of how the algorithm works.
...
The most commonly used kernels and their properties are presented in [the kernel cookbook](https://www.cs.toronto.edu/~duvenaud/cookbook/).
...
As for any GP method, initialisation of the hyper-parameters may have an influence on the final optimisation and lead to inadequate prediction for pathological cases.
...
pred_gp <- pred_gp(data=swimmers_pred,
                   grid_inputs = seq(10,16,0.1),
                   hp = model_gp,
                   kern = "SE",
                   plot = FALSE) 
```

With the [`plot_gp()`](https://arthurleroy.github.io/MagmaClustR/reference/plot_gp.html) function, we can display the evolution of Michaela’s predicted performances in more details. 
...
```
In the above figure, we can depict:

*  Michaela’s evolution curve (in purple) and the associated 95% credibility interval (in pink);

*   the training data points in the background with `data_train`. Each colour corresponds to one swimmer;

*   the mean GP as a dashed black line thanks to the `prior_mean` parameter.
...
The graphs below correspond to Michaela’s performances prediction thanks to *Magma* (left) and standard GP (right). 
...
gg1 <- plot_gp(pred_gp = pred,
  ...
  ggplot2::scale_x_continuous(limits = c(10,16))
