...
*   `shared_hp_k`: here, we assume that the set of hyper-parameters is the same for each cluster by setting `shared_hp_k = TRUE`. Thus, we suppose that all clusters share the same covariance structure. This property implies that the shapes and variations of the curves are assumed to be roughly identical from one cluster to another; the differentiation is mainly due to the mean values.

*  `shared_hp_i`: as we want to share information across children, we specify `shared_hp_i = TRUE`. If this assumption may appear restrictive at first glance, it actually offers a valuable way to share common patterns between tasks.
